...
import { Search, Plus, UserCheck, Calendar, ArrowLeft } from "lucide-react";
...
export default function PatientsPage() {
  ...
  const handleAddPatient = (e: React.FormEvent) => {
    e.preventDefault();
    const newId = `PT-${Math.floor(900 + Math.random() * 100)}`;
    const newPatient = {
      id: newId,
      name: newName,
      phone: newPhone,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-4">
        <div>
          <h1 className="text-2xl font-bold text-primary font-sans">دليل ملفات المرضى</h1>
          <p className="text-sm text-muted-foreground mt-1">
            البحث في السجلات الطبية والوصول لملف المريض وتاريخ الإعادات
          </p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          placeholder="ابحث باسم المريض، رقم الهاتف، أو رقم الملف (e.g. PT-901)..."
          ...
          <table className="w-full text-sm text-right">
            <thead className="bg-primary/5 text-primary font-semibold text-xs border-b border-border">
              <tr>
                ...
                <th className="p-3">عدد الزيارات / الإعادات</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md bg-card rounded-lg border border-border p-6 space-y-4 shadow-lg text-right">
            ...
            <form onSubmit={handleAddPatient} className="space-y-4 text-sm">
              <div>
                <label className="block text-xs font-semibold text-muted-foreground mb-1">اسم المريض رباعي</label>
                <input
                  type="text"
                  required
                  value={newName}
